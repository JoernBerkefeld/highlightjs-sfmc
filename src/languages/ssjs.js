...
export default function (hljs) {
    ...
    const CORE_OBJECT_METHODS = ['Init', 'Add', 'Remove', 'Update', 'Retrieve', 'Rows', 'Fields'];

    const PLATFORM_NAMESPACES = {
        scope: 'built_in',
        match: /\bPlatform\b(?=\s*\.)|(?<=Platform\.)\bFunction\b(?=\s*\.)|(?<=Platform\.)\bLoad\b(?=\s*\()|(?<=Platform\.)\bResponse\b(?=\s*\.)|(?<=Platform\.)\bRequest\b(?=\s*\.)|(?<=Platform\.)\bClientBrowser\b(?=\s*\.)/,
        relevance: 10,
    };
    ...
    const SFMC_GLOBAL_FUNCTION_RE = {
        match: [hljs.regex.concat(/\b/, hljs.regex.either(...SFMC_GLOBAL_FUNCTIONS)), /\s*(?=\()/],
        scope: { 1: 'built_in' },
        relevance: 8,
    };
